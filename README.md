...
|      EXPORT_FILENAME       |         Any string         |                                             The filename to be exported. This is actually templated, and you can use variables. Variables are wrapped in `{{$}}` (eg. `{{$USER_ID}}`), vars are "USER_ID", "USER_NAME", "CHANNEL_ID", "ID". Note that USER_ID & USER_NAME are only available if ID_TYPE = "USER". ID is just the ID you inputted, no matter the type                                             | "output/$ID" |
| EXPORT_HTML_DOWNLOAD_MEDIA |        Boolean type        |                                                                                                                                      Only available if EXPORT_TYPE = "HTML". If true, then it downloads all the media & attachments locally, otherwise it just links to it                                                                                                                                       |    false     |
|     EXPORT_HTML_THEME      |       "bad", "dark"        |                                                                                                                                     The theme of discord that will be used in the exported html project. "dark" is dark theme, "bad" is light theme. WHy would you do this?                                                                                                                                      |    "dark"    |
...
|    EXPORT_PLAIN_FORMAT     |         Any string         |                A template for how each msg is presented. vars are wrapped in `{{$VAR_HERE}}` (eg. `{{$AUTHOR_ID}}`). Available variables are: `AUTHOR_NAME`, `AUTHOR_ID`, `TIMESTAMP` (utc timestamp of the msg), `IS_REPLY` ("true" if it is replying to a msg), `WAS_EDITED`, `CONTENT`, `HAS_ATTACHMENT`, `ATTACHMENT_URL`, `IS_STICKER`, `STICKER_IDS` (a "," separated list of sticker IDs)                 |    string    |

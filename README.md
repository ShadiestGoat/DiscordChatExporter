...
| EXPORT_LOCATION | Any string | The location where this will be exported to. This will create a folder under that name that contains the media (see `DOWNLOAD_MEDIA`), and the messages. This is templated, and you can use variables. Variables are wrapped in `{{%VAR}}` (eg. `{{%CHANNEL_ID}}`), vars are `CHANNEL_ID`. | "output/%CHANNEL_ID" (cross-platform) |
